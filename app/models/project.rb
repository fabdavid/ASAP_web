...
#    Dir.mkdir(tmp_dir) if !File.exists?(tmp_dir)
    ...
    job = Basic.run_job(logger, cmd, self, self, 1, output_file, output_json, queue, self.parsing_job_id)
...
#      end
      ...
      job = Basic.run_job(logger, cmd, self, self, 2, output_file, output_json, queue, self.filtering_job_id)
...
#      Process.waitpid(pid)
      ...
      output_file = output_dir + 'output.tab'
      output_json = output_dir + 'output.json'
      job = Basic.run_job(logger, cmd, self, self, 3, output_file, output_json, queue, self.normalization_job_id)
