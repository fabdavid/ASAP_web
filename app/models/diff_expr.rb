...
#  belongs_to :step
  ...
  def run
    ...
    cmd = ([h_cmds[de_method.program], input_file, json_input_file, de_dir, de_method.name, group_file, selection_file, 1, 0] + list_p).join(" ") + " 1> #{de_dir}/output.log 2> #{de_dir}/output.err"
...
#    Process.waitpid(pid)
    ...
    job = Basic.run_job(logger, cmd, project, self, 6, output_json, output_json2, queue, self.job_id)
