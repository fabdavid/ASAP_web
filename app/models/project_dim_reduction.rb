...
#      output_json = visualization_dir + 'output.heatmap.json'
      cmd = "rake create_heatmap[#{self.id}]"
    

    elsif dr.id ==6
      
        cmd = "rake create_correlation[#{self.id}]"
      
    elsif dr.id ==7
      
      ### trajectory using sub_step param
        cmd = "rake create_trajectory[#{self.id}]"
    ...
    Basic.run_job(logger, cmd, project, self, 4, output_json, output_json, queue, self.job_id)
