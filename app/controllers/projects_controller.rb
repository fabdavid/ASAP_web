  ...
  def clone
    ...
    if @project.sandbox == false or admin?
      ...
      if current_user
        ...
        if p = Project.where(:key => session[:sandbox]).first
          delete_project(p)
        end
...
#        end
  ...
  def get_data_for_viz(dr_id)
    ...
    if !pdr
      ...
      pdr = ProjectDimReduction.new(:project_id => @project.id, :dim_reduction_id => dr_id)
      pdr.save
    end
    @active_item_name = @h_dim_reductions[session[:active_dr_id]].label.downcase
    return pdr
  end
...
#      end
  ...
  def get_step
   ...
   # session[:last_update_active_step]= @last_update
    ...
    if (current_user and current_user.id == @project.user_id) or admin? or @project.public == true or @project.sandbox == true
...
#        end
  ...
  def get_file
    ...
    if readable? @project
      ...
      send_file filepath.to_s, type: params[:content_type] || 'text', disposition: (!params[:display]) ? ("attachment; filename=" + params[:step] + "_" + filename) : ''
    end
  ...
  #  settings[:public_limit]=10
    ...
    base_search = (!settings[:free_text].empty?) ? Project.where("lower(name) ~ ?", settings[:free_text].downcase) : Project
    ...
    tmp_projects = (current_user) ? ((admin?) ? base_search : 
                                     base_search.where("user_id = ? or id = ?", current_user.id, shared_project_ids)) : 
...
#    end
  ...
  def manage_access
    ...
    existing_shares.each do |share|
      h_existing_users[share.user_id]=share
    end
    ...
    ## reset accesses
    existing_shares.each do |share|
      if !h_detected_users[share.user_id]
  ...
  def create
    ...
    if p = Project.where(:key => @project.key).first
      delete_project(p)
    end
